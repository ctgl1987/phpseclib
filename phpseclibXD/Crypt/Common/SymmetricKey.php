...
/**
 * Base Class for all \phpseclibXD\Crypt\* cipher classes
 *
 * PHP version 5
 *
 ...
 *
 *  - The new \phpseclibXD\Crypt\* cipher class should extend \phpseclibXD\Crypt\Common\SymmetricKey
 *
 ...
 *
 *  - Look at the source code of the current ciphers how they extend \phpseclibXD\Crypt\Common\SymmetricKey
...
namespace phpseclibXD\Crypt\Common;

use phpseclibXD\Common\Functions\Strings;
use phpseclibXD\Crypt\Hash;
use phpseclibXD\Exception\BadDecryptionException;
use phpseclibXD\Exception\BadModeException;
...
use phpseclibXD\Math\BinaryField;
use phpseclibXD\Math\PrimeField;
...
abstract class SymmetricKey
{
     ...
     *
     * @access private
     * @see \phpseclibXD\Crypt\Common\SymmetricKey::getEngine()
     */
    const ENGINE_MAP = [
     ...
     *
     * @see \phpseclibXD\Crypt\Twofish::setKey()
     * @see \phpseclibXD\Crypt\Twofish::setIV()
     * @var bool
     * @access private
     */
     ...
     * @access private
     * @param int $n
     * @param \phpseclibXD\Crypt\Hash $hashObj
     * @param string $i
     * @param string $d
     * @param int $count
     * @return string $a
     ...
     *
     * Put another way, when the continuous buffer is enabled, the state of the \phpseclibXD\Crypt\*() object changes after each
     ...
     * Encrypts a block
     *
     * Note: Must be extended by the child \phpseclibXD\Crypt\* class
     *
     * @access private
     * @param string $in
     * @return string
     ...
     *
     * Note: Must extend by the child \phpseclibXD\Crypt\* class
     *
     * @see self::setup()
     * @access private
     */
     ...
     *       - In case of using inline crypting, _setupInlineCrypt() must extend by the child \phpseclibXD\Crypt\* class.
     ...
     *    </code>
     *
     *    See also the \phpseclibXD\Crypt\*::_setupInlineCrypt()'s for
                    ...
                    if (strlen($_buffer["ciphertext"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= '.$block_size.') {
                            ...
                            if (strlen($_block) > strlen($_buffer["ciphertext"])) {
                                $in = $_xor;
                                '.$encrypt_block.'
                                \phpseclibXD\Common\Functions\Strings::increment_str($_xor);
                                $_buffer["ciphertext"].= $in;
                            }
                            $_key = \phpseclibXD\Common\Functions\Strings::shift($_buffer["ciphertext"], '.$block_size.');
                ...
                $encrypt = $init_encrypt . '
                    ...
                    if (strlen($_buffer["xor"])) {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= '.$block_size.') {
                            ...
                            $_key = \phpseclibXD\Common\Functions\Strings::shift($_buffer["xor"], '.$block_size.');
                            $_ciphertext.= $_block ^ $_key;
                        }
                    } else {
                        for ($_i = 0; $_i < $_plaintext_len; $_i+= '.$block_size.') {
