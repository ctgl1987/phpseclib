...
use phpseclibXD\Crypt\RSA\Formats\Keys\PSS;
use phpseclibXD\Exception\UnsupportedAlgorithmException;
use phpseclibXD\File\ASN1\Element;
use phpseclibXD\File\ASN1\Maps;
...
class X509
{
     ...
     *
     * @access public
     * @see \phpseclibXD\File\X509::getDN()
     */
    const DN_ARRAY = 0;
    /**
     * Return string
     ...
     *
     * @access public
     * @see \phpseclibXD\File\X509::saveX509()
     * @see \phpseclibXD\File\X509::saveCSR()
     * @see \phpseclibXD\File\X509::saveCRL()
     */
    const FORMAT_PEM = 0;
    /**
     * Save as DER
     ...
     *
     * There's no guarantee \phpseclibXD\File\X509 is going to re-encode an X.509 cert in the same way it was originally
    ...
    public function saveX509($cert, $format = self::FORMAT_PEM)
    {
        ...
        /* in the case of policyQualifiers/qualifier, the type has to be \phpseclibXD\File\ASN1::TYPE_IA5_STRING.
           \phpseclibXD\File\ASN1::TYPE_PRINTABLE_STRING will cause OpenSSL's X.509 parser to spit out random
           characters.
         */
                    ...
                    case 'id-ce-certificatePolicies':
                        for ($j = 0; $j < count($value); $j++) {
                            ...
                            for ($k = 0; $k < count($value[$j]['policyQualifiers']); $k++) {
                                ...
                                $subvalue = &$value[$j]['policyQualifiers'][$k]['qualifier'];
                                if ($map !== false) {
                                    // by default \phpseclibXD\File\ASN1 will try to render qualifier as a \phpseclibXD\File\ASN1::TYPE_IA5_STRING since it's
                                    // actual type is \phpseclibXD\File\ASN1::TYPE_ANY
    ...
    {
        if (!is_string($extnId)) { // eg. if it's a \phpseclibXD\File\ASN1\Element object
            return true;
        }
     ...
     * @access private
     * @throws \phpseclibXD\Exception\UnsupportedAlgorithmException if the algorithm is unsupported
     * @return bool
     */
     ...
     * Set public key
     *
     * Key needs to be a \phpseclibXD\Crypt\RSA object
     *
     * @param PublicKey $key
     * @access public
     * @return bool
     ...
     * Gets the public key
     *
     * Returns a \phpseclibXD\Crypt\RSA object or a false.
     *
     * @access public
     * @return mixed
     */
     ...
     *
     * @param \phpseclibXD\File\X509 $issuer
     * @param \phpseclibXD\File\X509 $subject
     * @access public
     * @return mixed
     */
    public function sign($issuer, $subject)
    {
        ...
        if (isset($subject->domains) && count($subject->domains)) {
            $altName = array_map(['\phpseclibXD\File\X509', 'dnsName'], $subject->domains);
        }
        ...
        // save $tbsCertificate in case there are any \phpseclibXD\File\ASN1\Element objects in it
    ...
    public function signCSR()
    {
        ...
        // resync $this->signatureSubject
        // save $certificationRequestInfo in case there are any \phpseclibXD\File\ASN1\Element objects in it
    ...
    public function signSPKAC()
    {
        ...
        // resync $this->signatureSubject
        // save $publicKeyAndChallenge in case there are any \phpseclibXD\File\ASN1\Element objects in it
     ...
     *
     * @param \phpseclibXD\File\X509 $issuer
     * @param \phpseclibXD\File\X509 $crl
     * @access public
     * @return mixed
     */
    public function signCRL($issuer, $crl)
    {
        ...
        // resync $this->signatureSubject
        // save $tbsCertList in case there are any \phpseclibXD\File\ASN1\Element objects in it
        $tbsCertList = $this->currentCert['tbsCertList'];
     ...
     * - Key object
     * - \phpseclibXD\File\X509 object with public or private key defined
     * - Certificate or CSR array
     * - \phpseclibXD\File\ASN1\Element object
     * - PEM or DER string
     *
    ...
    public function computeKeyIdentifier($key = null, $method = 1)
    {
        ...
        switch (true) {
            ...
            case $key instanceof X509:
                ...
                return false;
            default: // Should be a key object (i.e.: \phpseclibXD\Crypt\RSA).
